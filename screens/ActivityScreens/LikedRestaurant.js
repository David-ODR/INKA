...
import { Appbar, Searchbar, Avatar, Subheading, Card, Paragraph, Caption, Button, TouchableRipple, ActivityIndicator } from 'react-native-paper';

export default class LikedRestaurant extends React.Component {
  ...
  getlist = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        fetch(`http://Inkafoodapp.com/Api/Consumer/Coupon/NearMe/${position.coords.longitude},${position.coords.latitude}/${this.state.range}?Country=Philippines`,
          {
            headers: {
          ...
          ListEmptyComponent={() =>
            (
              <View style={{ flex: 1, justifyContent: "center", alignItems: "center", width: screenwidth, height: 200 }}>
                ...
                <Subheading>No Favorites Coupons</Subheading>
              </View>
            )}
          renderItem={({ item }) => (
            <Card style={styles.cardstyle} >
              <TouchableRipple
                onPress={() => this.props.navigation.navigate("Dealinfo", {
                  couponinfo: item
                })}
